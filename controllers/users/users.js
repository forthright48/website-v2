  ...
  *******************************************/
  ...
  function userList(req, res, next) {
    User.find({}, function(err, arr) {
      ...
      return world.myRender(req, res, 'users/userList', {
        data: arr
      });
    });
  }
    ...
    require('crypto').randomBytes(48, function(err, buffer) {
      ...
      const email = {
        ...
        text: 'Here is your activation link: ' +
          'http://www.forthright48.com/users/activate-account/${token}',
        html: `Here is your activation link:
          <a href="http://www.forthright48.com/users/activate-account/${token}">
          Activation Link </a>`
      };
      ...
      mailer.sendMail(email, function(err) {
        ...
        return world.myRender(req, res, 'success', {
          data: {
            message: 'Activation link has been sent. Please check your mail.' +
              ' Do not logout of your accout. Logging out will expire your activation link.'
          }
        });
      });
    });
  ...
  function activateAccount(req, res, next) {
    ...
    if (!req.session.activationToken) {
      return world.handleError(req, res, 'No activation link was sent to your mail');
    }
    const token = req.params.token;

    if (token !== req.session.activationToken) {
      return world.handleError(req, res, 'Activation token doesn\'t match ');
    }
